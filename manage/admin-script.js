...
import { formatDate, showModal, hideModal } from './admin-utils.js'; // استيراد الدوال المساعدة
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // -- Comments Management Selectors --
    const commentsTableContainer = document.getElementById('comments-table-container');
    const commentsLoadingMsg = document.getElementById('comments-loading');
    // #endregion
    ...
    async function loadDashboardStatsAndRecent() {
        ...
        try {
            ...
            const completedTodayQuery = query(transactionsRef, where("status", "==", "Completed"), where("timestamp", ">=", today));
    ...
    async function updateTransactionStatus(transactionDocId, newStatus) {
        ...
        try {
            ...
            const newStatusEntry = { status: newStatus, timestamp: new Date() };
            batch.update(transactionRef, { status: newStatus, statusHistory: arrayUnion(newStatusEntry), lastAdminUpdate: serverTimestamp() });
            if (newStatus === "Completed") {
                ...
                if (txDoc.exists()) {
                    ...
                    if (userId) {
                        ...
                        batch.update(userRef, { completedTransactionsCount: increment(1) });
                    }
                }
            }
            await batch.commit();
    ...
    async function loadTransactions(statusFilter = "all") {
        ...
        try {
            const transactionsRef = collection(db, "transactions");
            const q = (statusFilter === "all") ? query(transactionsRef, orderBy("timestamp", "desc")) : query(transactionsRef, where("status", "==", statusFilter), orderBy("timestamp", "desc"));
            ...
            querySnapshot.forEach(doc => {
                ...
                row.innerHTML = `
                    ...
                    <td><select class="status-change-select" data-id="${tx.id}"><option value="${currentStatus}" selected>${currentStatus}</option><option value="Pending">Pending</option><option value="Processing">Processing</option><option value="Completed">Completed</option><option value="Rejected">Rejected</option></select></td>
                `;
                row.querySelector('.status-change-select').addEventListener('change', (e) => {
                    ...
                    } else { e.target.value = currentStatus; }
                });
            });
            ...
            searchTransactionsInput.onkeyup = () => {
                ...
                tbody.querySelectorAll('tr').forEach(row => { row.style.display = row.textContent.toLowerCase().includes(searchTerm) ? "" : "none"; });
            };
        } catch (error) {
    ...
    // ==========================================================
    // SECTION: إدارة التعليقات (Comments Management)
    // ==========================================================
    async function handleDeleteComment(commentId, commentText) {
        if (confirm(`هل أنت متأكد من رغبتك في حذف التعليق التالي؟\n\n"${commentText.substring(0, 100)}..."\n\nسيتم حذف جميع الردود المرتبطة به.`)) {
            try {
                const batch = writeBatch(db);
                ...
                await batch.commit();
                alert("تم حذف التعليق والردود المرتبطة به بنجاح.");
                window.initializeCommentsManagementView();
            } catch (error) {
    ...
    async function renderCommentsTable(comments, users, articles) {
        ...
        const table = document.createElement('table');
        table.innerHTML = `<thead><tr><th>التعليق</th><th>الكاتب</th><th>المقال</th><th>التاريخ</th><th>الإجراءات</th></tr></thead><tbody></tbody>`;
        const tbody = table.querySelector('tbody');
        comments.sort((a,b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)).forEach(comment => {
            const authorName = comment.isAdminComment ? `<strong>Easy Exchange <span style="color:var(--primary-color)">(مشرف)</span></strong>` : usersMap.get(comment.userId) || 'مستخدم غير معروف';
            const articleTitle = articlesMap.get(comment.articleId) || 'مقال محذوف';
            const commentPreview = comment.commentText.substring(0, 60) + (comment.commentText.length > 60 ? '...' : '');
            const row = tbody.insertRow();
            row.style.background = comment.parentCommentId ? '#f8f9fa' : '#fff';
            row.innerHTML = `
                <td style="white-space: normal;">${comment.parentCommentId ? '<span style="margin-right:20px; color:#888;">⤷ رد: </span>' : ''}${commentPreview}</td>
                <td>${authorName}</td>
                <td><a href="../auth/article.html?id=${comment.articleId}" target="_blank">${articleTitle}</a></td>
                <td>${formatDate(comment.createdAt)}</td>
                <td><button class="button danger delete-comment-btn">حذف</button></td>
            `;
            row.querySelector('.delete-comment-btn').onclick = () => handleDeleteComment(comment.id, comment.commentText);
        });
    ...
    async function loadGlobalExchangeSettings() {
        ...
        } catch (error) { console.error("Error loading global settings:", error); globalSettings = {}; } 
        finally { globalSettingsLoadingMsg.style.display = 'none'; populateReportFilters(); }
    }
    ...
    async function saveGlobalExchangeSettings(event) {
        ...
        } catch (error) { alert(`فشل حفظ الإعدادات: ${error.message}`); }
    }
    ...
    async function savePaymentMethod(event) {
        ...
        const feesData = {
            sending: { type: pmSendingFeeTypeSelect.value, value: parseFloat(pmSendingFeeValueInput.value) || 0, minCap: parseFloat(pmSendingFeeMinCapInput.value) || null, maxCap: parseFloat(pmSendingFeeMaxCapInput.value) || null },
            receiving: { type: pmReceivingFeeTypeSelect.value, value: parseFloat(pmReceivingFeeValueInput.value) || 0, minCap: parseFloat(pmReceivingFeeMinCapInput.value) || null, maxCap: parseFloat(pmReceivingFeeMaxCapInput.value) || null }
        };
        let dataToSave = {
        ...
        try {
            if (documentId) { await updateDoc(doc(db, "paymentMethods", documentId), dataToSave); } 
            else { await addDoc(collection(db, "paymentMethods"), dataToSave); }
    ...
    async function loadManagedPaymentMethods() {
        ...
        try {
            ...
            if (querySnapshot.empty) { paymentMethodsContainer.innerHTML = "<p style='text-align:center; padding: 20px;'>لم يتم إضافة وسائل دفع بعد.</p>"; return; }
    ...
    async function saveMarqueeSettings(event) {
        event.preventDefault();
        const settingsData = { marqueeMessages: mqMessagesTextarea.value.split('\n').filter(Boolean), marqueeAnimationSpeedFactor: parseFloat(mqSpeedInput.value), lastUpdatedMarquee: serverTimestamp() };
        try {
    ...
    function calculatePlatformFee(transaction, usdtBuyRate) {
        ...
        if (isNaN(sendAmount) || sendAmount <= 0) return 0;
        let feeInBaseCurrency = sendAmount * PLATFORM_FEE_PERCENTAGE;
        let feeInEGP = 0;
        if (sendCurrencyType === 'EGP') { feeInEGP = feeInBaseCurrency; } 
        else if (sendCurrencyType === 'USDT') {
            if (isNaN(usdtBuyRate) || usdtBuyRate <= 0) { console.warn(`USDT buy rate is not available or invalid (${usdtBuyRate}).`); return 0; }
    ...
    async function loadFeesReport(month, year) {
        ...
        try {
            ...
            if (!globalSettings.usdtToEgpBuyRate) { await loadGlobalExchangeSettings(); }
            ...
            const querySnapshot = await getDocs(q);
            let totalCompletedCount = 0, totalFeesEGP = 0;
            ...
            if (reportTransactions.length === 0) { feesTableContainer.innerHTML = ''; feesNoDataMsg.style.display = 'block'; } 
            else {
                feesNoDataMsg.style.display = 'none';
                const table = document.createElement('table');
    ...
    function initializeAdminPanel() { if (window.adminSwitchView) window.adminSwitchView('dashboard-view'); }
    window.initializeAdminDashboardView = () => { loadDashboardStatsAndRecent(); loadManagedPaymentMethods(); };
    window.initializeTransactionsView = () => { loadTransactions("all"); };
    window.initializeSettingsView = async () => { await loadGlobalExchangeSettings(); await loadMarqueeSettings(); };
    window.initializePaymentMethodsView = loadManagedPaymentMethods;
    window.initializeFeesReportView = () => { populateReportFilters(); loadGlobalExchangeSettings(); };

    window.initializeCommentsManagementView = async () => {
        if (!commentsLoadingMsg || !commentsTableContainer) return;
        commentsLoadingMsg.style.display = 'block';
        commentsTableContainer.innerHTML = '';
        commentsTableContainer.appendChild(commentsLoadingMsg);
        try {
            const [commentsSnap, usersSnap, articlesSnap] = await Promise.all([
                getDocs(query(collection(db, "comments"))),
                getDocs(query(collection(db, "users"))),
                getDocs(query(collection(db, "articles")))
            ]);
            const comments = commentsSnap.docs.map(d => ({id: d.id, ...d.data()}));
            const users = usersSnap.docs.map(d => ({ uid: d.id, ...d.data(), name: `${d.data().firstName || ''} ${d.data().lastName || ''}`.trim() || d.data().email }));
            const articles = articlesSnap.docs.map(d => ({id: d.id, ...d.data()}));
            if (comments.length > 0) {
                await renderCommentsTable(comments, users, articles);
            } else {
                commentsTableContainer.innerHTML = '<p style="text-align:center; padding: 20px;">لا توجد تعليقات لعرضها حاليًا.</p>';
            }
        } catch (error) {
            console.error("Error initializing comments view:", error);
            commentsTableContainer.innerHTML = `<p style="color:red; text-align:center; padding: 20px;">فشل تحميل التعليقات: ${error.message}</p>`;
        } finally {
            commentsLoadingMsg.style.display = 'none';
        }
    };
    ...
    loadFeesReportBtn?.addEventListener('click', () => {
        ...
        if (!isNaN(selectedMonth) && !isNaN(selectedYear)) { loadFeesReport(selectedMonth, selectedYear); } 
        else { alert("الرجاء اختيار شهر وسنة صالحين."); }
    });
