...
// #endregion
    ...
    function checkAdminAuth() {
        if (sessionStorage.getItem('adminAuthenticated') === 'true') {
            ...
            // Prompt will happen only when navigating to admin/index.html without session
            // If the login box has a button to trigger the prompt, add listener here
             document.getElementById('prompt-login-button')?.addEventListener('click', requestAdminCredentials);
        }
    }

    // Trigger credential prompt on page load if not authenticated
    // Uncomment the line below if you want the prompt to appear automatically on page load
    if (sessionStorage.getItem('adminAuthenticated') !== 'true') {
        // requestAdminCredentials(); // Auto-prompt on load
    }
    ...
    function requestAdminCredentials() {
        ...
        if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
            ...
            checkAdminAuth(); // Re-check auth to show content and initialize
        } else {
            alert("بيانات الاعتماد غير صحيحة. الرجاء المحاولة مرة أخرى.");
            requestAdminCredentials(); // Prompt again on failure
        }
    }
    ...
    async function loadTransactions(statusFilter = "all") {
        ...
        try {
            ...
            const table = document.createElement('table');
             // Removed "سجل الحالات" column to revert to original state
            table.innerHTML = `
                <thead>
                    <tr>
                        <th>ID العملية</th>
            ...
            allTransactionsData.forEach(tx => {
                ...
                const statusClassSuffix = currentStatus.toLowerCase().replace(/\s+/g, '-');
                 // Use original timestamp for main transactions list date
                ...
                // Removed status history cell content
                ...
                `;
                // Event listener for status change dropdown is delegated
            });
    ...
    transactionsTableContainer?.addEventListener('change', (e) => {
        if (e.target.classList.contains('status-change-select')) {
            ...
            if (confirm(`هل أنت متأكد من تغيير الحالة إلى ${selectedStatus}؟`)) {
                ...
                // Revert the dropdown value if user cancels
        ...
        if (globalSettingsCache) {
         ...
         try {
            ...
            if (docSnap.exists()) {
                ...
                console.error("Global settings document not found.");
                return null;
            }
        } catch (error) {
    ...
    async function saveGlobalExchangeSettings(event) {
        ...
        if (isNaN(buyRate) || isNaN(sellRate) || buyRate <= 0 || sellRate <= 0) {
         ...
         if (gsUsdtServiceFeeTypeSelect.value !== 'none' && (isNaN(feeValue) || feeValue < 0)) {
             alert("الرجاء إدخال قيمة عددية موجبة للرسوم.");
             return;
         }
          if (gsUsdtServiceFeeTypeSelect.value === 'percentage') {
             if (feeMinCap !== null && (isNaN(feeMinCap) || feeMinCap < 0)) { alert("الرجاء إدخال قيمة عددية موجبة للحد الأدنى للرسوم."); return; }
             if (feeMaxCap !== null && (isNaN(feeMaxCap) || feeMaxCap < 0)) { alert("الرجاء إدخال قيمة عددية موجبة للحد الأقصى للرسوم."); return; }
          }
        ...
        // Clean up nulls for empty strings or NaN values explicitly based on type
         if (settingsData.usdtToUsdtServiceFeeType !== 'percentage') {
             ...
             settingsData.usdtToUsdtServiceFeeMaxCap = null;
              ...
              if (isNaN(feeMinCap)) settingsData.usdtToUsdtServiceFeeMinCap = null;
              if (isNaN(feeMaxCap)) settingsData.usdtToUsdtServiceFeeMaxCap = null;
         }
         if (settingsData.usdtToUsdtServiceFeeType === 'none') {
             settingsData.usdtToUsdtServiceFeeValue = 0;
             settingsData.usdtToUsdtServiceFeeMinCap = null;
             settingsData.usdtToUsdtServiceFeeMaxCap = null;
         }
        ...
        try {
            ...
            globalSettingsCache = settingsData; // Update cache
    ...
    function toggleFeeFields(feeTypeSelect, valueGroup, valueInput, maxCapGroup, maxCapInput, minCapGroup, minCapInput) {
        ...
        // Only make inputs required if the type is 'fixed' or 'percentage'
        (valueInput ?? {required:false}).required = (selectedType !== 'none');
        // Min/Max cap are only applicable and potentially required for 'percentage'
        // We don't make them *required* in HTML here, rely on JS validation on save if needed.


        if (selectedType === "fixed") {
            (valueGroup ?? {style:{}}).style.display = 'block';
        } else if (selectedType === "percentage") {
            (valueGroup ?? {style:{}}).style.display = 'block';
            (minCapGroup ?? {style:{}}).style.display = 'block';
            (maxCapGroup ?? {style:{}}).style.display = 'block';
        }
    }
    ...
    function openPaymentMethodModal(method = null) {
        ...
        const pmDocumentIdInput = document.getElementById('pm-document-id'); // Get inside function to ensure exists
        ...
        if (pmDocumentIdInput) pmDocumentIdInput.value = ''; // Clear ID for new method
        ...
        if (method) {
            ...
            modalTitle.textContent = "إضافة وسيلة دفع جديدة";
             if(keyInput) keyInput.readOnly = false; // Key is editable for new method
    ...
    async function savePaymentMethod(event) {
        ...
        // Fee related inputs inside the modal - get references here
         ...
         // Validate required fields for new method and basic fields for all
        ...
        const balanceVal = balanceStr === '' ? null : parseFloat(balanceStr);
         ...
         // Validate numbers
         if (minAmountVal !== null && isNaN(minAmountVal)) { alert("الحد الأدنى يجب أن يكون رقمًا."); return; }
         if (balanceVal !== null && isNaN(balanceVal)) { alert("الرصيد يجب أن يكون رقمًا."); return; }
        ...
        const feesData = {
         ...
         ['sending', 'receiving'].forEach(feeKey => {
             ...
             // Ensure value is a number, default 0 if type is not none and value is NaN/empty
             if (feeConfig.type !== 'none' && isNaN(feeConfig.value)) feeConfig.value = 0;
             if (feeConfig.type === 'none') feeConfig.value = 0;

             // Min/Max caps are only relevant for percentage
             if (feeConfig.type !== 'percentage') {
                 feeConfig.minCap = null;
                 feeConfig.maxCap = null;
             } else {
                  // For percentage, if cap is NaN/empty string, make it null
                  if (isNaN(feeConfig.minCap)) feeConfig.minCap = null;
                  if (isNaN(feeConfig.maxCap)) feeConfig.maxCap = null;
                  // If type is percentage, value should be > 0
                  if (feeConfig.value <= 0) { alert(`قيمة الرسوم لـ ${feeKey === 'sending' ? 'الإرسال' : 'الاستلام'} (النسبة) يجب أن تكون موجبة.`); throw new Error("Invalid fee value"); } // Throw error to stop save
                  // If type is percentage, min/max caps must be non-negative if provided
                   if (feeConfig.minCap !== null && feeConfig.minCap < 0) { alert(`الحد الأدنى للرسوم لـ ${feeKey === 'sending' ? 'الإرسال' : 'الاستلام'} يجب أن يكون غير سالب.`); throw new Error("Invalid fee min cap"); }
                   if (feeConfig.maxCap !== null && feeConfig.maxCap < 0) { alert(`الحد الأقصى للرسوم لـ ${feeKey === 'sending' ? 'الإرسال' : 'الاستلام'} يجب أن يكون غير سالب.`); throw new Error("Invalid fee max cap"); }

             }
             // Ensure fixed fee value is > 0 if type is fixed
              if (feeConfig.type === 'fixed' && feeConfig.value <= 0) { alert(`قيمة الرسوم لـ ${feeKey === 'sending' ? 'الإرسال' : 'الاستلام'} (الثابتة) يجب أن تكون موجبة.`); throw new Error("Invalid fee value"); } // Throw error to stop save
        ...
        if (dataToSave.isSiteAccount) {
             ...
             // Remove site account specific fields if not a site account
             delete dataToSave.recipientInfo;
             delete dataToSave.recipientType;
        }


        try {
            if (documentId) {
                // Key is readOnly in UI for edit, so no need to check if changing.
                // Just ensure we don't accidentally write it if it was set on dataToSave (it shouldn't be).
                 const updateData = { ...dataToSave };
                 // No need to delete key here as it's not added to dataToSave on edit anyway.

                await updateDoc(doc(db, "paymentMethods", documentId), updateData);
            } else {
            ...
            loadManagedPaymentMethods(); // Reload list
        } catch (error) {
            console.error("Error saving payment method:", error);
             // Check if the error was thrown by our validation checks (using specific messages)
            if (error.message.includes("Invalid fee value") || error.message.includes("Invalid fee min cap") || error.message.includes("Invalid fee max cap")) {
                // Specific error message already alerted within the fee loop
            } else {
    ...
    async function loadManagedPaymentMethods() {
        ...
        try {
            ...
            if(activeMethodsCountSpan) activeMethodsCountSpan.textContent = allPaymentMethodsData.filter(d => d.isActive).length;
             ...
             paymentMethodsContainer.querySelector('#payment-methods-loading-temp')?.remove(); // Hide loading message
         ...
         paymentMethodForm?.addEventListener('change', (e) => {
             if (e.target.id === 'pm-sending-fee-type' || e.target.id === 'pm-receiving-fee-type') {
                 ...
                 if (targetId === 'pm-sending-fee-type') {
                      valueGroup = document.querySelector('.pm-sending-fee-value-group');
                      valueInput = document.getElementById('pm-sending-fee-value');
                      minCapGroup = document.querySelector('.pm-sending-fee-mincap-group');
                      minCapInput = document.getElementById('pm-sending-fee-mincap');
                      maxCapGroup = document.querySelector('.pm-sending-fee-maxcap-group');
                      maxCapInput = document.getElementById('pm-sending-fee-maxcap');
                 } else { // pm-receiving-fee-type
                      valueGroup = document.querySelector('.pm-receiving-fee-value-group');
                      valueInput = document.getElementById('pm-receiving-fee-value');
                      minCapGroup = document.querySelector('.pm-receiving-fee-mincap-group');
                      minCapInput = document.getElementById('pm-receiving-fee-mincap');
                      maxCapGroup = document.querySelector('.pm-receiving-fee-maxcap-group');
                      maxCapInput = document.getElementById('pm-receiving-fee-maxcap');
                 }
