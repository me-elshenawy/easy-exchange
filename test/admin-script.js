...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // #region ======== Constants ========
    const PLATFORM_FEE_PERCENTAGE = 0.005; // 0.5% (NEW)
    // #endregion

    // Global variable to store settings, including USDT rate (NEW)
    let globalSettings = {};
    ...
    function updateDashboardAndStats(transactions) {
        ...
        // Update active payment methods count (assuming loadManagedPaymentMethods updates the span with ID active-methods-count)
        // If not, we'd need to pass that count here or read it after loading methods.
        // Let's assume loadManagedPaymentMethods updates #active-methods-count.

        const recentTableBody = document.querySelector('#recent-transactions-table tbody');
        if (recentTableBody) { // Check if the element exists
    ...
    async function loadTransactions(statusFilter = "all") {
        ...
        try {
            ...
            updateDashboardAndStats(transactionsArray); // Update dashboard stats with ALL transactions initially
    ...
    async function loadGlobalExchangeSettings() {
        ...
        try {
            ...
            if (docSnap.exists()) {
                globalSettings = docSnap.data(); // Store settings globally
                ...
                gsUsdtServiceFeeMaxCapInput.value = globalSettings.usdtToUsdtServiceFeeMaxCap ?? '';
                 ...
                 // Initialize globalSettings if document doesn't exist
                 globalSettings = {};
            }
            ...
            console.error("Error loading global settings:", error);
             globalSettings = {}; // Ensure globalSettings is initialized even on error
        } finally {
            globalSettingsLoadingMsg.style.display = 'none';
             // Populate year and month dropdowns for report regardless of settings load success
             populateReportFilters(); // NEW
        }
    }
    ...
    async function saveGlobalExchangeSettings(event) {
        ...
        try {
            await setDoc(doc(db, "exchangeSettings", GLOBAL_SETTINGS_DOC_ID), settingsData, { merge: true });
             globalSettings = { ...globalSettings, ...settingsData }; // Update global variable immediately (NEW)
    ...
    function toggleFeeFields(feeTypeSelect, valueGroup, valueInput, maxCapGroup, maxCapInput, minCapGroup, minCapInput) {
        ...
        [valueGroup, maxCapGroup, minCapGroup].forEach(g => {
             if (g) g.style.display = 'none' // Check if group exists
        });
        if (valueInput) valueInput.required = false; // Check if input exists
    ...
    function openPaymentMethodModal(method = null) {
        ...
        if (method) {
            ...
            // START: Populate Balance Field on Edit
            document.getElementById('pm-balance').value = method.balance ?? '';
            // END: Populate Balance Field on Edit
            ...
            // Clear balance field for new method
            document.getElementById('pm-balance').value = ''; // NEW
        }
    ...
    async function savePaymentMethod(event) {
        ...
        let dataToSave = {
            ...
            // START: Add Balance to Save Object
            balance: balanceVal,
            // END: Add Balance to Save Object
    ...
    async function loadManagedPaymentMethods() {
        ...
        try {
            ...
            // Update active methods count in dashboard stats (NEW)
            const activeMethodsCountSpan = document.getElementById('active-methods-count');
            if (activeMethodsCountSpan) {
                 activeMethodsCountSpan.textContent = querySnapshot.docs.filter(d => d.data().isActive).length;
            }
            ...
            querySnapshot.forEach((docSnapshot) => {
                ...
                // START: Modify Card HTML to Display Balance
                card.innerHTML = `
                    ...
                    <div class="actions" style="margin-top: auto; padding-top: 15px; border-top: 1px solid var(--border-color); display: flex; gap: 10px;">
                        <button class="button secondary edit-pm-btn">تعديل</button>
                        <button class="button danger delete-pm-btn">حذف</button>
                    </div>
                `;
                // END: Modify Card HTML to Display Balance
    ...
    // ==============================================
    // SECTION: Platform Fees Report (NEW)
    // ==============================================

     // New function to populate month/year dropdowns (NEW)
    function populateReportFilters() {
        ...
        reportMonthSelect.value = new Date().getMonth() + 1; // Select current month

        // Populate Years (e.g., last 5 years + current + next 1)
        const currentYear = new Date().getFullYear();
        const startYear = currentYear - 5; // Go back 5 years
        const endYear = currentYear + 1;   // Include next year for forward planning
        reportYearSelect.innerHTML = '';
        for (let i = currentYear; i >= startYear; i--) { // Start from current year and go back
             reportYearSelect.innerHTML += `<option value="${i}">${i}</option>`;
        }
         reportYearSelect.value = currentYear; // Select current year
    }

     /**
     * @description Calculates the platform fee for a single completed transaction.
     * Fee is 0.5% of the send amount, converted to EGP if necessary. (NEW)
    ...
    function calculatePlatformFee(transaction, usdtBuyRate) {
        // Only calculate for completed transactions (though the query already filters this)
        ...
        if (sendCurrencyType === 'EGP') {
            ...
            if (isNaN(usdtBuyRate) || usdtBuyRate <= 0) {
                console.warn(`USDT buy rate is not available or invalid (${usdtBuyRate}). Cannot convert USDT fee to EGP for transaction ${tx.transactionId || tx.id}.`);
                 return 0; // Cannot convert if rate is invalid
            }
            feeInEGP = feeInBaseCurrency * usdtBuyRate;
        } else {
            // For any other currency type, the fee basis is not defined as EGP or USDT send amount
            // As per requirement: If currency is not EGP or USDT, fee is 0.
            return 0;
        }
    ...
    /**
     * @description Loads, calculates, and displays the fees report for a given month and year. (NEW)
    ...
    async function loadFeesReport(month, year) {
        ...
        try {
            ...
            const transactionsRef = collection(db, "transactions");
            // Query completed transactions within the date range based on creation timestamp
            // using the 'timestamp' field as requested alternative.
            const q = query(
                transactionsRef,
                where("status", "==", "Completed"),
                where("timestamp", ">=", startDate),
                where("timestamp", "<", endDate), // Use '<' the start of the next month
                orderBy("timestamp", "asc") // orderBy is required for range queries on timestamp
            );
            ...
            // Get the current USDT buy rate from loaded global settings
            // Use 0 or a fallback if settings or rate are missing
            const usdtBuyRate = globalSettings.usdtToEgpBuyRate || 0;
             if (usdtBuyRate <= 0) {
                 console.warn("USDT buy rate is not available or is 0. USDT fees will be calculated as 0 EGP.");
             }
            ...
            querySnapshot.forEach(docSnapshot => {
                ...
                tx.platformFeeEGP = fee; // Add calculated fee to the transaction object
            ...
            feesSummaryStatsDiv.style.display = 'grid'; // Show the stats grid

            // Display details table or no data message
            if (reportTransactions.length === 0) {
                feesTableContainer.innerHTML = ''; // Clear any old table
                ...
                table.innerHTML = `
                    <thead>
                        <tr>
                            ...
                            <th>تاريخ الإنشاء</th> <!-- Using creation date as per query -->
                            <th>رسوم المنصة (EGP)</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${reportTransactions.map(tx => {
                            const creationTimestampDisplay = tx.timestamp?.toDate() ? tx.timestamp.toDate().toLocaleString('ar-EG-u-nu-latn') : 'غير محدد';
                            return `
                                <tr>
                                    <td>${tx.transactionId || 'N/A'}</td>
                                    <td>${tx.exchangerName || tx.userId || 'غير معروف'}</td>
                                    <td>${(tx.sendAmount || 0).toFixed(2)} ${tx.sendCurrencyName || tx.sendCurrencyType || ''}</td>
                                    <td>${(tx.receiveAmount || 0).toFixed(2)} ${tx.receiveCurrencyName || tx.receiveCurrencyType || ''}</td>
                                    <td>${creationTimestampDisplay}</td>
            ...
            feesLoadingMsg.style.display = 'none';
            feesSummaryStatsDiv.style.display = 'none'; // Hide stats on error too
            feesTableContainer.innerHTML = '';
            feesNoDataMsg.style.display = 'none'; // Hide no data message too
            ...
            errorMessageDiv.textContent = `فشل تحميل تقرير الرسوم: ${error.message}`;
            feesTableContainer.appendChild(errorMessageDiv); // Display error in table area
        }
    }
    ...
    function initializeAdminPanel() {
        // Load initial data for different sections
        loadTransactions(); // Loads transactions and updates dashboard transaction stats
        loadGlobalExchangeSettings(); // Loads settings, populates report filters, and stores settings globally
        loadManagedPaymentMethods(); // Loads payment methods and updates active methods count on dashboard
        loadMarqueeSettings();
        ...
        // Event listeners for fee type selects (existing)
        [pmSendingFeeTypeSelect, pmReceivingFeeTypeSelect, gsUsdtServiceFeeTypeSelect].forEach(select => {
            select?.addEventListener('change', (e) => {
                ...
                } else { // Global settings fee select
        ...
        // Event listener for the Load Fees Report button (NEW)
    ...
    } // End of initializeAdminPanel


    // Call the initial authentication check when the DOM is fully loaded
    checkAdminAuth();
