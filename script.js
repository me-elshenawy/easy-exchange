    ...
    async function initializeUserAccountStatus() {
        ...
        try {
            ...
            onAuthStateChanged(auth, async (user) => {
                ...
                const isContactPage = window.location.pathname.includes('/contact.html'); // New contact page check
                ...
                const contactLinkPath = window.location.pathname.startsWith('/auth/') ? '../contact.html' : 'contact.html'; // Path to the new contact page
                ...
                if (user && user.emailVerified) {
                    ...
                    navHtml = `
                        <a href="${contactLinkPath}" class="nav-link ${isContactPage ? 'active' : ''}">
                            <box-icon name='message-square-detail'></box-icon> <!-- تم إضافة هذه الأيقونة -->
                    ...
                    navHtml = `
                        <a href="${contactLinkPath}" class="nav-link ${isContactPage ? 'active' : ''}">تواصل معنا</a>
            ...
            const contactLinkPathFallback = window.location.pathname.startsWith('/auth/') ? '../contact.html' : 'contact.html';
            mainNavContainer.innerHTML = `<a href="${contactLinkPathFallback}" class="nav-link">تواصل معنا</a> <a href="${helpLinkPathFallback}" class="nav-link">مساعدة</a> <a href="/auth/login.html" class="header-button primary">تسجيل الدخول</a>`;
        }
    }
...
سأقوم بإرسال إيصال التحويل و ${userIdentifierForChat} الآن.`;
    ...
    window.addEventListener('resize', updateLayoutPadding); // Ensures padding adjusts on window resize
